...
locals {
  ...
  common_tags = {
    ...
    Repository = "instantencore-digital-program-book-workspace/network"
  ...
  production_access_cidr_blocks = [
    local.known_cidrs.office_santa_clara,
    local.known_cidrs.office_san_diego,
    local.known_cidrs.home_zach,
    local.known_cidrs.home_evan,
  ]

  dev_access_cidr_blocks = concat(local.production_access_cidr_blocks, [
    local.known_cidrs.home_alfredo,
    local.known_cidrs.home_alfredo2,
  ])

  whitelist_access_cidr_blocks = local.is_production ? local.production_access_cidr_blocks : local.dev_access_cidr_blocks
}
